          ...
          <div class="hidden lg:ml-4 lg:flex lg:items-center lg:pr-0.5">
            ...
            <div class="ml-4 relative flex-shrink-0">
              ...
              <transition
                ...
                enter-class="transform opacity-100 scale-100"
                enter-to-class="transition ease-in duration-75"
              ...
              <div class="flex items-center px-5">
                <div class="flex-shrink-0">
                  <img
                    ...
                    src="https://images.unsplash.com/photo-1550525811-e5869dd03032?ixlib=rb-1.2.1&ixqx=pTgr0tKWAM&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"
                    alt=""
                  />
                </div>
              ...
              <div class="rounded-lg bg-white overflow-hidden shadow">
                <div class="p-6">
                  <div v-if="isEditMode" class="text-right">
                    <router-link
                      to="/"
                      class="flex items-center mb-4 inline-flex border border-gray-300 rounded-md px-4 py-1 hover:bg-gray-100 transition duration-150 hover:border-blue-100"
                    >
                      <span>
                        Add New
                      </span>
                      <span
                        class="flex-shrink-0 bg-white inline-flex h-8 w-8 items-center justify-center rounded-full border-2 border-dashed border-gray-200 text-gray-400 hover:text-gray-500 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ml-4"
                      >
                        <svg
                          class="h-3 w-3"
          ...
          <div class="grid grid-cols-1 gap-4">
            <section aria-labelledby="section-2-title">
              ...
              <div class="rounded-lg bg-white overflow-hidden shadow">
                <div class="p-6">
                  <div>
                    <h3 class="font-medium text-gray-900">Other Notes</h3>
                  </div>
                  <ul
                  ...
                  >
                    ...
                    >
                      ...
                      <router-link
                        :to="`/notes/${note.id}`"
                        class="-mr-2 h-8 w-8 bg-white rounded-full flex items-center justify-center text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                        <svg
                          class="h-5 w-5"
      ...
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 lg:max-w-7xl">
        ...
        >
          <span class="block sm:inline"
            >Github Repo
            <a
              class="block sm:inline text-blue-500"
              href="https://github.com/sandiprb/supabase-vue3-tailwind-typescript"
            >
              Here
            </a></span
          >
...
<script lang="ts">
import useNote from "@/use/useNote";
import { handleLogout } from "@/use/useAuth";
import {
  computed,
  defineComponent,
  onMounted,
...
export default defineComponent({
  ...
  setup() {
    ...
    const isEditMode = computed(() => !!params.id);
    ...
    const { createNote, userNotes, fetchNoteDetails, updateNote } = useNote();
    const submit = async () => {
      if (isEditMode.value) {
        await updateNote({ ...toRaw(form), id: params.id as string });
      } else {
        const { data } = await createNote(toRaw(form));
        if (data) {
          form.title = "";
          form.content = "";
        }
    ...
    onMounted(async () => {
      if (params.id) {
        const { data }: { data: any } = await fetchNoteDetails(
          params?.id as string
        );
